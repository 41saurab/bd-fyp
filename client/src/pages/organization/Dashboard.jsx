...
export default function OrgDashboard() {
  ...
  useEffect(() => {
    axios.get('/api/organizations/dashboard').then(r => setData(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  if (isPending) return (
    <div className="max-w-2xl mx-auto px-6 py-20 text-center">
      ...
      <p className="text-stone-500 font-body">Your organization <strong>{org?.orgName}</strong> is under review. Our admin team will verify your documents within 24-48 hours.</p>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/organization/campaigns" className="text-xs text-crimson font-sans hover:underline">View all</Link>
          </div>
          {!(data?.recentCampaigns?.length) ? (
            <div className="text-center py-8">
              <p className="text-stone-400 font-sans text-sm mb-3">No campaigns yet</p>
              <Link to="/organization/campaigns/create" className="btn-primary text-sm">Create First Campaign</Link>
            </div>
          ) : (
              ...
              {data.recentCampaigns.map(c => (
                <div key={c._id} className="flex items-center gap-3 p-3 rounded-xl bg-stone-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400 font-sans">{c.city} · {c.status}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold font-sans text-crimson">{c.collectedUnits}/{c.targetUnits}</p>
                    <p className="text-xs text-stone-400 font-sans">units</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/organization/emergency" className="text-xs text-crimson font-sans hover:underline">View all</Link>
          </div>
          {!(data?.recentEmergencies?.length) ? (
            <div className="text-center py-8">
              <p className="text-stone-400 font-sans text-sm mb-3">No emergency requests</p>
              <Link to="/organization/emergency/create" className="btn-primary text-sm">Post Emergency</Link>
            </div>
          ) : (
            <div className="space-y-3">
              {data.recentEmergencies.map(e => (
                <div key={e._id} className="flex items-center gap-3 p-3 rounded-xl bg-stone-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400 font-sans capitalize">{e.urgencyLevel} · {e.status}</p>
                  </div>
                  <div className="text-right">
      ...
      <div className="card p-6 mt-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/organization/inventory" className="btn-outline text-sm">Update</Link>
        </div>
