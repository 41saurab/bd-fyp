    ...
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-blood-900 via-crimson to-blood-700 items-center justify-center p-16 relative overflow-hidden">
        <div className="absolute inset-0">
          {[...Array(6)].map((_, i) => (
            <svg key={i} viewBox="0 0 24 24" className="absolute fill-white/5 animate-float"
              style={{ width: `${60 + i * 30}px`, left: `${10 + i * 15}%`, top: `${10 + (i % 3) * 25}%`, animationDelay: `${i * 0.5}s` }}>
              <path d="M12 2C12 2 4 10.5 4 15a8 8 0 0016 0C20 10.5 12 2 12 2z"/>
            </svg>
          ))}
        </div>
        <div className="relative text-center text-white max-w-sm">
          ...
          <div className="mt-10 grid grid-cols-3 gap-4 text-center">
            {[['50K+', 'Donors'], ['8K+', 'Hospitals'], ['1M+', 'Lives Saved']].map(([n, l]) => (
              <div key={l}>
        ...
        <div className="w-full max-w-md">
            ...
            <div>
              ...
              <div className="relative">
                <input {...register('password', { required: 'Password required' })}
                  type={showPass ? 'text' : 'password'} className="input-field pr-10" placeholder="Enter password" />
          ...
          <div className="mt-8 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link to="/register/donor" className="btn-outline text-sm text-center py-2.5">Register as Donor</Link>
              <Link to="/register/organization" className="btn-ghost text-sm text-center py-2.5 border border-stone-200 rounded-lg">Register Organization</Link>
            </div>
          </div>

          <div className="mt-8 p-4 bg-red-50 rounded-xl border border-red-100">
            <p className="text-xs text-stone-500 font-sans font-semibold mb-2">Demo Accounts:</p>
            <div className="space-y-1 text-xs text-stone-500 font-sans">
              <p>Admin: admin@bloodbridge.com / Admin@123</p>
            </div>
          </div>
        </div>
      </div>
