      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-6">
            <svg viewBox="0 0 24 24" className="w-8 h-8 fill-crimson animate-heartbeat"><path d="M12 2C12 2 4 10.5 4 15a8 8 0 0016 0C20 10.5 12 2 12 2z"/></svg>
            <span className="font-display font-bold text-2xl text-stone-800">Blood<span className="text-crimson">Bridge</span></span>
          </Link>
        ...
        <div className="card p-8">
            ...
            <div className="border-b border-stone-100 pb-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input {...register('name', { required: 'Required' })} className="input-field" placeholder="Dr. John Smith" />
                  {errors.name && <p className="text-red-500 text-xs mt-1 font-sans">{errors.name.message}</p>}
                </div>
                <div>
                  <label className="label">Email Address *</label>
                  <input {...register('email', { required: 'Required', pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: 'Invalid email' } })} className="input-field" placeholder="org@hospital.com" />
                ...
                <div>
                  ...
                  <input {...register('phone', { required: 'Required' })} className="input-field" placeholder="+1 234 567 8900" />
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input {...register('orgName', { required: 'Required' })} className="input-field" placeholder="City General Hospital" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input {...register('registrationNumber', { required: 'Required' })} className="input-field" placeholder="REG-2024-XXXX" />
                    {errors.registrationNumber && <p className="text-red-500 text-xs mt-1 font-sans">{errors.registrationNumber.message}</p>}
                  </div>
                  <div>
                    <label className="label">City *</label>
                    <input {...register('city', { required: 'Required' })} className="input-field" placeholder="New York" />
                    {errors.city && <p className="text-red-500 text-xs mt-1 font-sans">{errors.city.message}</p>}
                  </div>
                </div>
                <div>
                  <label className="label">Address *</label>
                  <input {...register('address', { required: 'Required' })} className="input-field" placeholder="123 Medical Center Blvd" />
                  {errors.address && <p className="text-red-500 text-xs mt-1 font-sans">{errors.address.message}</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">Contact Person</label>
                    <input {...register('contactPerson')} className="input-field" placeholder="Dr. Smith" />
                  </div>
                  <div>
                    <label className="label">Contact Phone</label>
                    <input {...register('contactPhone')} className="input-field" placeholder="Direct line" />
                  </div>
                </div>
                <div>
                  <label className="label">Website</label>
                  <input {...register('website')} className="input-field" placeholder="https://hospital.com" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${docFile ? 'border-green-300 bg-green-50' : 'border-stone-200 hover:border-crimson hover:bg-red-50'}`}
          ...
          <p className="text-center text-sm text-stone-500 mt-5 font-sans">
            Already registered? <Link to="/login" className="text-crimson hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
