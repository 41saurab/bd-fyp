...
export default function EmergencyDetail() {
  ...
  useEffect(() => {
    axios.get(`/api/emergency/${id}`).then(r => setReq(r.data)).catch(() => navigate('/emergency')).finally(() => setLoading(false));
  }, [id]);
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <a href={`tel:${req.contactPhone}`} className="text-sm font-sans text-crimson font-medium hover:underline">{req.contactPhone}</a>
              </div>
              <div className="flex items-start gap-2">
                <MapPin className="w-4 h-4 text-crimson mt-0.5" />
                <span className="text-sm font-sans text-stone-600">{req.location}, {req.city}</span>
              </div>
            </div>
          </div>
            ...
            {req.status === 'active' && (
              hasResponded ? (
                ...
                <button onClick={handleRespond} disabled={responding}
                  className="w-full py-3 bg-crimson hover:bg-blood-800 text-white font-sans font-semibold rounded-xl transition-colors emergency-pulse">
