...
export function Organizations() {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/organizations?${params}`).then(r => setOrgs(r.data || [])).finally(() => setLoading(false));
  }, [filter]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      {loading ? <div className="grid grid-cols-3 gap-6">{[...Array(6)].map((_, i) => <div key={i} className="card p-6 animate-pulse h-32"></div>)}</div> : (
