		...
		<nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-stone-100 shadow-sm">
			<div className="max-w-7xl mx-auto px-4 sm:px-6">
				<div className="flex items-center justify-between h-16">
					...
					<Link to="/" className="flex items-center gap-2.5 group">
						...
						<div>
							<span className="font-display font-bold text-xl text-stone-800">
								Rakta<span className="text-crimson">bindu</span>
							</span>
						</div>
					</Link>
					...
					<div className="hidden md:flex items-center gap-1">
						{navLinks.map((link) => (
							<Link key={link.to} to={link.to} className={clsx("px-3 py-2 rounded-lg text-sm font-sans font-medium transition-colors", isActive(link.to) ? "text-crimson bg-red-50" : "text-stone-600 hover:text-crimson hover:bg-red-50")}>
								{link.label}
							</Link>
						))}
					</div>
					...
					<div className="hidden md:flex items-center gap-3">
						{user ? (
							<div className="relative">
								<button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-stone-50 transition-colors">
								...
								{profileOpen && (
									<div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-xl shadow-lg border border-stone-100 py-2 z-50">
										...
										<button
											...
											className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 font-sans transition-colors"
										>
							...
							<>
								...
								<Link to="/register/donor" className="btn-primary text-sm">
									Become a Donor
								</Link>
							</>
						)}
			...
			{mobileOpen && (
				<div className="md:hidden bg-white border-t border-stone-100 px-4 py-4 space-y-1">
					{navLinks.map((link) => (
						<Link key={link.to} to={link.to} onClick={() => setMobileOpen(false)} className={clsx("block px-3 py-2.5 rounded-lg text-sm font-sans", isActive(link.to) ? "text-crimson bg-red-50" : "text-stone-600")}>
							{link.label}
						</Link>
					))}
					<div className="pt-3 border-t border-stone-100 mt-3 flex flex-col gap-2">
						{user ? (
							<>
								<Link to={getDashboardLink()} onClick={() => setMobileOpen(false)} className="btn-outline text-sm text-center">
									Dashboard
								</Link>
								<button
									onClick={() => {
							...
							<>
								<Link to="/login" onClick={() => setMobileOpen(false)} className="btn-outline text-sm text-center">
									Login
								</Link>
								<Link to="/register/donor" onClick={() => setMobileOpen(false)} className="btn-primary text-sm text-center">
									Become a Donor
								</Link>
							</>
						)}
