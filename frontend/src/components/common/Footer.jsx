		...
		<footer className="bg-stone-900 text-stone-300">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-10">
					...
					<div>
						...
						<ul className="space-y-2 text-sm font-sans">
							{[
								["Register as Donor", "/register/donor"],
								...
								<li key={to}>
									<Link to={to} className="hover:text-blood-400 transition-colors">
										{label}
									</Link>
								</li>
							))}
						...
						<div className="mt-6">
							...
							<div className="flex flex-wrap gap-1.5 mt-2">
								{["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map((bt) => (
									<span key={bt} className="text-xs px-2 py-0.5 bg-stone-800 text-stone-300 rounded-full font-sans">
										{bt}
									</span>
								))}
							</div>
			...
			<div className="border-t border-stone-800">
				<div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-2">
					<p className="text-xs text-stone-500 font-sans">© {new Date().getFullYear()} Raktabindu. Saving lives together.</p>
