	...
	return (
		<div className="max-w-5xl mx-auto px-6 py-8">
			<div className="flex items-center justify-between mb-8">
				...
				<Link to="/organization/campaigns/create" className="btn-primary flex items-center gap-2">
			...
			{loading ? (
				...
				<div className="text-center py-20">
					...
					<Link to="/organization/campaigns/create" className="btn-primary">
				...
				<div className="space-y-4">
					{campaigns.map((c) => (
						<Link key={c._id} to={`/organization/campaigns/${c._id}`} className="card p-5 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer">
							...
							<div className="flex-1 min-w-0">
								...
								<div className="flex flex-wrap gap-3 text-xs text-stone-400 font-sans">
									...
									<span className="flex items-center gap-1">
										...
										{c.registeredDonors?.length || 0} donors
									</span>
								</div>
								<div className="flex flex-wrap gap-1 mt-2">
									{c.targetBloodTypes?.slice(0, 4).map((bt) => (
										<BloodTypeBadge key={bt} type={bt} />
									))}
								</div>
							</div>
							<div className="text-right flex-shrink-0">
								<p className="text-xl font-display font-bold text-crimson">{c.registeredDonors?.length || 0}</p>

								<p className="text-xs text-stone-400 font-sans">of {c.targetUnits} donors</p>

								<div className="w-24 h-1.5 bg-stone-100 rounded-full mt-2">
									<div
										className="h-full bg-crimson rounded-full"
										style={{
											width: `${Math.min(100, ((c.registeredDonors?.length || 0) / c.targetUnits) * 100)}%`,
										}}
									/>
								</div>
							</div>
