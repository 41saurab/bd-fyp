...
import { MapPin, Calendar, Users, Clock, Award, Phone, Droplet } from "lucide-react";
...
export default function CampaignDetail() {
	...
	const handleRegister = async () => {
		...
		try {
			...
			setCampaign((c) => ({ ...c, isRegistered: true, registeredDonors: [...(c.registeredDonors || []), { status: "registered" }] }));
		} catch (err) {
	...
	const isRegistered = campaign?.isRegistered ?? false;
		...
		<div className="max-w-5xl mx-auto px-6 py-10">
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<div className="lg:col-span-2 space-y-6">
					<div className="card overflow-hidden">
						<div className="h-56 bg-gradient-to-br from-blood-800 to-crimson relative flex items-center justify-center">
							{campaign.image ? (
								<img src={`${campaign.image}`} alt="" className="w-full h-full object-fit" />
							) : (
								<svg viewBox="0 0 24 24" className="w-24 h-24 fill-white/20">
				...
				<div className="space-y-4">
					{/* Register CTA — donors (and logged-out guests, who get a login
              prompt) only. Orgs/admins manage this campaign from their own
              dashboard, not from the public donate flow. */}
					{["upcoming", "active"].includes(campaign.status) && (!user || user.role === "donor") && (
						<div className="card p-6">
					...
					{campaign.organization && (
						<div className="card p-5">
							...
							<p className="text-xs text-stone-400 font-sans capitalize mt-1">
								{campaign.organization.orgType?.replace("_", " ")} · {campaign.organization.city}
							</p>
						</div>
					)}

					{/* Donor roster — only present when the backend actually sent full
              donor identities (the owning org, or an admin). Everyone else
              only gets the count shown above in Campaign Progress. */}
					{campaign.registeredDonors?.some((r) => r.donor && typeof r.donor === "object") && (
						<div className="card p-6">
							<h3 className="font-display font-semibold text-stone-800 mb-4 flex items-center gap-2">
								<Users className="w-4 h-4 text-crimson" /> Registered Donors ({campaign.registeredDonors.length})
							</h3>
							<div className="space-y-3">
								{campaign.registeredDonors.map((r, i) => {
									const donor = r.donor;
									if (!donor || typeof donor !== "object") return null;
									return (
										<div key={donor._id || i} className="flex items-center gap-3 p-3 rounded-xl border border-stone-100">
											...
											<span className="text-xs px-2 py-0.5 rounded-full font-sans font-medium bg-stone-100 text-stone-500 capitalize">{r.status}</span>
										</div>
									);
								})}
							</div>
