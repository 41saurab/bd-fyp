...
export const updateEmergencyDTO = Joi.object({
	patientName: Joi.string().optional().label("patientName"),
	bloodType: Joi.string().valid("A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-").optional().label("bloodType"),
	unitsNeeded: Joi.number().optional().label("unitsNeeded"),
	urgencyLevel: Joi.string().valid("critical", "urgent", "moderate").optional().label("urgencyLevel"),
	reason: Joi.string().optional().label("reason"),
	location: Joi.string().optional().label("location"),
	city: Joi.string().optional().label("city"),
	contactPerson: Joi.string().optional().label("contactPerson"),
	contactPhone: Joi.string().min(10).max(10).optional().label("contactPhone"),
	deadline: Joi.date().allow(null).optional().label("deadline"),
	additionalNotes: Joi.string().allow("", null).optional().label("additionalNotes"),
	latitude: Joi.number().min(-90).max(90).allow(null, "").optional().label("latitude"),
	longitude: Joi.number().min(-180).max(180).allow(null, "").optional().label("longitude"),
